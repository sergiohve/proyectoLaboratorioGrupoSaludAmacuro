...
const RegistroExamen = () => {
    ...
    const fetchClientes = async () => {
      try {
        const response = await fetch(
          `${API_BASE}/api/clientes?all=true`
        );
        if (response.ok) {
  ...
  const handleEliminarTipoCustom = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tipos-examen/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleGuardarPlantilla = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tipos-examen`, {
        method: "POST",
